...
if __name__ == "__main__":
    ...
    prob.model.add_design_var('heat_sink.t_fin', lower=0.001, upper=0.1)
    prob.model.add_design_var('heat_sink.Ht', lower=0, upper=0.1)
    prob.model.add_design_var('heat_sink.Wth', lower=0, upper=0.5)
    prob.model.add_design_var('heat_sink.Lng', lower=0, upper=0.4)
    prob.model.add_design_var('heat_sink.N_fins', lower=1)
    prob.model.add_design_var('W', lower=0.0001)

    prob.model.add_subsystem('Q_calc', om.ExecComp('T_b=Q*R_th_tot+T_air', T_b={'units':'K'}, T_air={'units':'K'}, Q={'units':'W'}, R_th_tot={'units':'K/W'}), promotes=['*'])

    prob.model.add_constraint('T_b', upper=273+150) # 150 degC
    prob.model.add_constraint('heat_sink.dPqP', upper=1.0)
    prob.model.add_subsystem('obj', om.ExecComp('obj = Wt', Wt={'units':'kg'}), promotes=['*'])

    prob.model.connect('heat_sink.Wt', 'Wt')
    # prob.model.connect('heat_sink.dPqP', 'dPqP')
    prob.model.connect('heat_sink.R_th_tot', 'R_th_tot')

    prob.model.add_objective('obj', ref=1.0)

    prob.setup()
    Q = 3.
    prob.set_val('Q', val=Q, units='kW')
    prob.set_val('W', val=0.085, units='kg/s')
    prob.set_val('T_air', val=63, units='degC')
    #prob.set_val('T', val=(350-273.15), units='degC')
    prob.set_val('P', val=101.325, units='kPa')
    prob.set_val('heat_sink.Ht', val=0.041, units='m' )
    prob.set_val('heat_sink.Wth', val=0.1795, units='m')
    prob.set_val('heat_sink.Lng', val=0.3886, units='m')
    prob.set_val('heat_sink.t_fin', val=.001, units='m')
    prob.set_val('heat_sink.N_fins', val=43, units=None)
    ...
    print('T_air =',prob.get_val('T_air'))
    print('T_b =',prob.get_val('T_b', units='degC'))
